...
RUN set -eux; \
    ...
    cp ${BUILD_DIR}/external/db_drivers/liblmdb/liblmdb.a ${WORK_DIR}/output/x86_64/lib/ 2>/dev/null || true; \
    cp ${BUILD_DIR}/src/blocks/libblocks.a ${WORK_DIR}/output/x86_64/lib/ 2>/dev/null || true; \
    cp ${BUILD_DIR}/src/device_trezor/libdevice_trezor.a ${WORK_DIR}/output/x86_64/lib/ 2>/dev/null || true; \
    cp ${BUILD_DIR}/src/multisig/libmultisig.a ${WORK_DIR}/output/x86_64/lib/ 2>/dev/null || true; \
    cp ${BUILD_DIR}/src/net/libnet.a ${WORK_DIR}/output/x86_64/lib/ 2>/dev/null || true; \
    cp ${BUILD_DIR}/src/hardforks/libhardforks.a ${WORK_DIR}/output/x86_64/lib/ 2>/dev/null || true; \
    cp ${BUILD_DIR}/src/rpc/librpc_base.a ${WORK_DIR}/output/x86_64/lib/ 2>/dev/null || true; \
    cp ${BUILD_DIR}/src/cryptonote_basic/libcryptonote_format_utils_basic.a ${WORK_DIR}/output/x86_64/lib/ 2>/dev/null || true; \
    cp ${BUILD_DIR}/src/libversion.a ${WORK_DIR}/output/x86_64/lib/ 2>/dev/null || echo "[WARN] libversion.a missing"; \
    \
    # Critical missing libraries 
    echo "=== Copying additional critical libraries ==="; \
    # RandomX library 
    cp ${BUILD_DIR}/external/randomx/librandomx.a ${WORK_DIR}/output/x86_64/lib/ 2>/dev/null || echo "[WARN] librandomx.a not found"; \
    # Wallet crypto library 
    cp ${BUILD_DIR}/src/crypto/wallet/libwallet-crypto.a ${WORK_DIR}/output/x86_64/lib/ 2>/dev/null || echo "[WARN] libwallet-crypto.a not found"; \
    # Character set library (from external libs) 
    find ${BUILD_DIR}/external -name "libcharset.a" -exec cp {} ${WORK_DIR}/output/x86_64/lib/ \; 2>/dev/null || echo "[WARN] libcharset.a not found"; \
    \
    # External libraries 
    ...
    cp ${BOOST_ROOT}/lib/libboost_wserialization.a ${WORK_DIR}/output/x86_64/lib/ 2>/dev/null || echo "[WARN] libboost_wserialization.a missing"; \
    \
